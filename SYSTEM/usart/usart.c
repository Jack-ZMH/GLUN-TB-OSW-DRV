...
// UART接收数组 、指令临时缓存数组
uint8_t UartBuff[SIZE],CmdBuffer[SIZE];
// 解析标记、uart缓存计数、指令临时缓存数组下表、数组同步标志
uint8_t Cmd_flag , Cnt_Buff , Cmd_Index ,Syn_Cnt;
...
void USART1_IRQHandler(void) 
{
	...
	if(__HAL_UART_GET_FLAG(&UART1_Handler,UART_FLAG_RXNE)!= RESET)
	{
		...
		res = (uint16_t) READ_REG(UART1_Handler.Instance->DR);//接收命令
		//WRITE_REG(UART1_Handler.Instance->DR,res); //串口调试使用回显
		UartBuff[Cnt_Buff++] = res;
		if(UartBuff[Cnt_Buff-2] == 0x0D && UartBuff[Cnt_Buff-1] == 0x0A) //必须要结尾为0XOA、0X0D才行
		{
				Cmd_Index = 0;
				for(i=Syn_Cnt;i != Cnt_Buff;i++) //数据同步拷贝
				{
					if(i > SIZE-1) i = 0;
					CmdBuffer[Cmd_Index++] = UartBuff[i];//数据拷贝
				}
