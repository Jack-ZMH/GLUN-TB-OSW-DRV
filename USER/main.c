		...
		if(Switch == ON)
		{
			switch(TYPEFEF_SELECT)
			{
				case C1X2G	:{
								...
								C1X2G_CHENCK(Cmd_Parame.timer,1,Cmd_Parame.type);
							 }break;
				case D2X2B	:{
					            Cmd_Parame.type = LATCH;
								D2X2B_CHENCK(Cmd_Parame.timer,1);
							 }break;
				case M1X1	:{
								Cmd_Parame.type = LATCH;
								M1X1_CHENCK(Cmd_Parame.timer,1);
							 }break;
				case M1X4	:{
					 			Cmd_Parame.type = LATCH;
								M1X4_CHENCK(Cmd_Parame.timer,1,Cmd_Parame.type);
							 }break;
				case S1X4	:{
								Cmd_Parame.type = LATCH;
								S1X4_CHENCK(Cmd_Parame.timer,1,Cmd_Parame.type);
							 }break;
				case S1X8	:{
								Cmd_Parame.type = LATCH;
								Latch_S1X8_CHENCH(Cmd_Parame.timer,1,Cmd_Parame.type);break;
				             }break;
				case N_C1X2G:{
				...
				case N_S1X8	:{
					            Cmd_Parame.type = NONLATCH;
							    Latch_S1X8_CHENCH(Cmd_Parame.timer,1,Cmd_Parame.type);
				             }break;
...
void C1X2G_StateSwitch(CMD_Parame *parame){
	...
	{
	switch(parame->state){
		case A:NonLatch_C1X2_StateA(20);break;
		case B:NonLatch_C1X2_StateB(20);break;
		default:printf("C1X2G State Error\n");
			break;
	}
	}
